...
class FRF:
    ...
    def readExcel(self):
        ...
        self.dis_max = max(self.avg_dis[80:400])
        self.dis_index = self.Freq[np.argmax(self.avg_dis[80:400])]

        self.Amp_max = max(self.avg_Amp[80:400])
        self.Amp_index = self.Freq[np.argmax(self.avg_Amp[80:400])]
...
def compare_compliance(filename, axis):
    ...
    for i in range(0, len(filename), 2):
        ...
        if opt.max >= non_opt.max:
            optimized_number+=1
        non_opt_max_list.append(non_opt.dis_max)
        opt_max_list.append(opt.dis_max)
        ...
        # ax.scatter(non_opt.dis_index+40, non_opt.dis_max)
        # ax.scatter(opt.dis_index+40, opt.dis_max)
    ...
    ax.tick_params(axis="y", direction="in", which='minor', labelsize=13)
    print("here?2")
    print("shape:", len(index), len(non_opt_max_list))
    rect1 = ax.bar(index, non_opt_max_list, width, color = 'r' )
    rect2 = ax.bar(index+width, opt_max_list, width, color = 'b' )
...
if __name__ == "__main__":
    ...
    non_opt_comp_y, opt_comp_y = compare_compliance(filename_y, "y")
    non_opt_comp_x, opt_comp_x = compare_compliance(filename_x, "x")

    non_opt_sum, opt_sum = [], []
    for i in range(len(non_opt_comp_x)):
        non_opt_sum.append(non_opt_comp_y[i] +  non_opt_comp_x[i])
        opt_sum.append(opt_comp_y[i] +  opt_comp_x[i])
    ...
    ax.set_title("Impact hammer test result", fontsize=17)
    ax.set_xlabel('Posture'  , fontsize = 16)
    ax.set_ylabel('Sum of Compliance [m/N]', fontsize = 16)
    ...
    rect1 = ax.bar(index, non_opt_sum, width, color = 'r' )
